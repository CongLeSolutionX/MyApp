...
final class RealOpenAIBackend: ChatBackend {
    ...
    func streamChat(messages: [Message], systemPrompt: String, completion: @escaping (Result<String, Error>) -> Void) {
        let allMessages = systemPrompt.isEmpty ? messages : [.system(systemPrompt)] + messages
...
final class CoreMLChatBackend: ChatBackend {
    ...
    func streamChat(messages: [Message], systemPrompt: String, completion: @escaping (Result<String, Error>) -> Void) {
        guard coreModel != nil else { return completion(.failure(NSError(domain: "CoreMLError", code: 1, userInfo: [NSLocalizedDescriptionKey: "CoreML model '\(modelName)' could not be loaded."]))) }
...
final class ChatStore: ObservableObject {
    ...
    //    return TakeoverVUIView(store: previewStore, speech: SpeechRecognizer())
    ...
    //    return TakeoverVUIView(store: previewStore, speech: previewSpeech)
