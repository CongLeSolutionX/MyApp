...
/*
 Make sure to add these keys and descriptions to your Info.plist:
...
//        let micStatus = AVAudioSession.sharedInstance().recordPermission
    ...
    private func presentRecognitionError(_ error: Error?) async {
        ...
        // Ignore common cancellation errors for better UX
    ...
    static var previews: some View {
        VStack(spacing: 15) {
            GroupBox("Preview Controls") {
                Picker("Voice Mode", selection: $previewVoiceManager.currentMode) {
                    ForEach(VoiceMode.allCases) { mode in
                        Text(mode.rawValue).tag(mode)
                    }
                }
                .pickerStyle(.segmented)
                .onChange(of: previewVoiceManager.currentMode) {
            ...
            VoiceInputAreaView(
                ...
                sendMessageAction: {
                    print("Send (\(previewVoiceManager.currentMode.rawValue)): \(previewUserInput)")
                    previewUserInput = ""
                },
        ...
        .onAppear {
            previewVoiceManager.currentMode = .liveTranscription
        }
    }
}
