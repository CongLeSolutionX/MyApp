...
final class HeraDataService: ObservableObject  {
    ...
    private func getAccessToken(completion: @escaping (Result<String, HERAAPIError>) -> Void) {
        ...
        let credentials = "\(HERAAuthCredentials.clientID):\(HERAAuthCredentials.clientSecret)"
        ...
        URLSession.shared.dataTaskPublisher(for: request)
            .tryMap { data, response -> Data in
                ...
                case 400:  // Specifically check for bad request, can indicate problems in your request (e.g., invalid credentials)
                    ...
                    throw HERAAPIError.requestFailed("Bad Reqeuest. Response: \(responseString)")
                ...
                case 500...599:       // Server errors
                    ...
                    throw HERAAPIError.serverError("Server error: \(httpResponse.statusCode), Response: \(responseString)") //Specific error
            ...
            .decode(type: HERAAPI_TokenResponse.self, decoder: JSONDecoder())
        ...
        URLSession.shared.dataTaskPublisher(for: request)
            .tryMap { data, response -> Data in  //Correctly checks for error codes
                ...
                case 200...299:   // Handle Successful Status Codes
                    return data
                case 401:  // Unauthorized
                    throw HERAAPIError.unauthorized
                case 403:    // Forbidden
                    throw HERAAPIError.forbidden
                case 404:   //Not Found
                    let responseString = String(data: data, encoding: .utf8) ?? "No Response Body"
                    throw HERAAPIError.requestFailed("Resource not found - 404 Not Found. Response: \(responseString)")
                    
                    // Additional handling of Error statuses.
                case 500...599:  // Handle Server Error statuses
                    let body = String(data: data, encoding: .utf8) ?? "No Response Body"
                    throw HERAAPIError.serverError("Server Error: \(httpResponse.statusCode) , Details: \(body)")
    ...
    private func pageNumber(for endpoint: HERAAPIEndpoint) -> Int
    {
        switch endpoint {
        case .all( let page):
            return page
    ...
    func handleError(_ error: HERAAPIError) {
