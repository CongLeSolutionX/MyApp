...
enum GatewayServicesPublicAPIEndpoint {
    case getAllUsers
...
enum GatewayServicesPublicAPIError: Error, LocalizedError {
    case invalidURL
...
struct GatewayServicesPublicAuthCredentials {
...
final class GatewayService: ObservableObject {
    ...
    private func getAccessToken(completion: @escaping (Result<String, GatewayServicesPublicAPIError>) -> Void) {
        ...
        let credentials = "\(GatewayServicesPublicAuthCredentials.clientID):\(GatewayServicesPublicAuthCredentials.clientSecret)"
        ...
        URLSession.shared.dataTaskPublisher(for: request)
            .tryMap { data, response -> Data in
                guard let httpResponse = response as? HTTPURLResponse,
                    ...
                    throw GatewayServicesPublicAPIError.requestFailed("Invalid response. Response: \(responseString)")
                }
                return data
            }
            ...
            .sink { [weak self] completionResult in
                ...
                case .failure(let error):
                    let apiError = (error as? GatewayServicesPublicAPIError) ?? GatewayServicesPublicAPIError.unknown(error)
    ...
    func fetchData(for endpoint: GatewayServicesPublicAPIEndpoint) {
        isLoading = true
        errorMessage = nil
    ...
    private func makeDataRequest(endpoint: GatewayServicesPublicAPIEndpoint, accessToken: String) {
        ...
        URLSession.shared.dataTaskPublisher(for: request)
            .tryMap { data, response in
                guard let httpResponse = response as? HTTPURLResponse,
                    ...
                    throw GatewayServicesPublicAPIError.requestFailed("HTTP Status Code error. Response: \(responseString)")
                }
                return data
            }
            .decode(type: [User].self, decoder: JSONDecoder()) // Decode directly to [User]
            .catch { [weak self] error -> AnyPublisher<[User], Never> in
                if let apiError = error as? GatewayServicesPublicAPIError {
                    self?.handleError(apiError)
                } else {
    ...
    private func handleError(_ error: GatewayServicesPublicAPIError) {
        DispatchQueue.main.async {
            self.isLoading = false
