...
struct ArticleView: View {
    ...
    func makeAttributedString(from string: String) -> AttributedString {
        // 1. Create the full AttributedString first
        var attributedString = AttributedString(string)
        
        // 2. Define the substring you want to find and format
        let substringToFormat = "`NavigationDestination`"
        
        // 3. Find the range of that substring *within the AttributedString*
        if let rangeInAttributedString = attributedString.range(of: substringToFormat) {
            // 4. Apply formatting to the found AttributedString range
            attributedString[rangeInAttributedString].foregroundColor = .yellow
            attributedString[rangeInAttributedString].font = .system(.body, design: .monospaced)
            attributedString[rangeInAttributedString].backgroundColor = .gray.opacity(0.3)
            
            // Optional: If you want to remove the backticks after formatting
            // Be careful if your formatting relies on the backticks being there initially.
            // You might need to adjust the range or re-apply formatting slightly differently.
            // attributedString.replaceSubrange(rangeInAttributedString, with: "NavigationDestination")
            
        }
        // 5. Return the potentially modified AttributedString
