...
//    // Note: We access by index directly here. If using indexed drawing,
//    // the GPU handles fetching the correct vertex based on the index buffer.
//    // The vid passed here corresponds to the *index value* read from the
//    // index buffer during an indexed draw call.
...
///// Renamed to `ShapeVertex` to be slightly more generic, but describes the same layout.
//struct ShapeVertex {
...
//    // --- Geometry Data ---
//
//    /// Array defining the 12 vertices of the Icosahedron. Uses the golden ratio (phi).
//    /// Coordinates are based on (0, ±1, ±phi) and its permutations, scaled slightly.
//    let vertices: [ShapeVertex] = {
//        let phi = (1.0 + sqrt(5.0)) * 0.5 // Golden ratio
//        let r: Float = 1.0 // Radius or scaling factor
//
//        // Create vertices using permutations of (0, ±1, ±phi)
//        return [
//            // Vertex Format: ShapeVertex(position: SIMD3<Float>(x, y, z), color: SIMD4<Float>(r, g, b, a))
//            // Assign distinct colors for visualization
//            ShapeVertex(position: SIMD3<Float>(-1,  phi,    0) * r, color: SIMD4<Float>(1.0, 0.0, 0.0, 1)), // 0 Red
//            ShapeVertex(position: SIMD3<Float>( 1,  phi,    0) * r, color: SIMD4<Float>(0.0, 1.0, 0.0, 1)), // 1 Green
//            ShapeVertex(position: SIMD3<Float>(-1, -phi,    0) * r, color: SIMD4<Float>(0.0, 0.0, 1.0, 1)), // 2 Blue
//            ShapeVertex(position: SIMD3<Float>( 1, -phi,    0) * r, color: SIMD4<Float>(1.0, 1.0, 0.0, 1)), // 3 Yellow
//
//            ShapeVertex(position: SIMD3<Float>( 0, -1,  phi) * r, color: SIMD4<Float>(1.0, 0.0, 1.0, 1)), // 4 Magenta
//            ShapeVertex(position: SIMD3<Float>( 0,  1,  phi) * r, color: SIMD4<Float>(0.0, 1.0, 1.0, 1)), // 5 Cyan
//            ShapeVertex(position: SIMD3<Float>( 0, -1, -phi) * r, color: SIMD4<Float>(1.0, 0.5, 0.0, 1)), // 6 Orange
//            ShapeVertex(position: SIMD3<Float>( 0,  1, -phi) * r, color: SIMD4<Float>(0.5, 0.0, 1.0, 1)), // 7 Purple
//
//            ShapeVertex(position: SIMD3<Float>( phi,  0, -1) * r, color: SIMD4<Float>(0.0, 0.5, 0.5, 1)), // 8 Teal
//            ShapeVertex(position: SIMD3<Float>( phi,  0,  1) * r, color: SIMD4<Float>(0.5, 1.0, 0.5, 1)), // 9 Light Green
//            ShapeVertex(position: SIMD3<Float>(-phi,  0, -1) * r, color: SIMD4<Float>(0.5, 0.5, 0.0, 1)), // 10 Olive
//            ShapeVertex(position: SIMD3<Float>(-phi,  0,  1) * r, color: SIMD4<Float>(0.0, 0.5, 1.0, 1))  // 11 Sky Blue
//        ]
//    }() // Use immediately invoked closure to calculate phi once
//
...
//    let indices: [UInt16] = [
//        // 5 faces around vertex 0 (Top-like cluster)
//        0, 11, 5,
//        0, 5, 1,
//        0, 1, 7,
//        0, 7, 10,
//        0, 10, 11,
//
//        // 5 adjacent faces connecting the two "poles"
//        1, 5, 9,
//        5, 11, 4,
//        11, 10, 2,
//        10, 7, 6,
//        7, 1, 8,
//
//        // 5 faces around vertex 3 (Bottom-like cluster)
//        3, 9, 4,
//        3, 4, 2,
//        3, 2, 6,
//        3, 6, 8,
//        3, 8, 9,
//
//        // 5 adjacent faces connecting to the bottom points
//        4, 9, 5,
//        2, 4, 11,
//        6, 2, 10,
//        8, 6, 7,
...
//            // Describes ShapeVertex layout (Position + Color)
...
//    func setupBuffers() {
//        // --- Vertex Buffer ---
//        let vertexDataSize = vertices.count * MemoryLayout<ShapeVertex>.stride
//        guard vertexDataSize > 0, // Ensure vertices array is not empty
//              let vBuffer = device.makeBuffer(bytes: vertices, length: vertexDataSize, options: []) else {
//            fatalError("Could not create vertex buffer. Vertex count: \(vertices.count)")
//        }
...
//        guard indexDataSize > 0, // Ensure indices array is not empty
//              let iBuffer = device.makeBuffer(bytes: indices, length: indexDataSize, options: []) else {
//            fatalError("Could not create index buffer. Index count: \(indices.count)")
//        }
//        indexBuffer = iBuffer
//        indexBuffer.label = "Icosahedron Indices"
//
//        // --- Uniform Buffer ---
//        let uniformBufferSize = MemoryLayout<Uniforms>.size // Needs to be at least matrix size
//        guard let uBuffer = device.makeBuffer(length: max(uniformBufferSize, 16), options: .storageModeShared) else { // Ensure min size
...
//        )
//        // View Matrix (Same as before)
//        let viewMatrix = matrix_look_at_left_hand(
//            eye: SIMD3<Float>(0, 0.5, -4), center: SIMD3<Float>(0, 0, 0), up: SIMD3<Float>(0, 1, 0)
//        )
//        // Model Matrix (Rotation - Same as before)
...
//
//    /// Called whenever the `MTKView`'s drawable size changes.
//    func mtkView(_ view: MTKView, drawableSizeWillChange size: CGSize) {
//        aspectRatio = Float(size.width / max(1, size.height))
//        print("Icosahedron MTKView Resized - New Aspect Ratio: \(aspectRatio)")
//    }
//
//    /// Called for each frame to encode rendering commands for the Icosahedron.
...
//
//        // Bind Buffers for Icosahedron
//        renderEncoder.setVertexBuffer(vertexBuffer, offset: 0, index: 0)
//        renderEncoder.setVertexBuffer(uniformBuffer, offset: 0, index: 1)
//
//        // **** Draw Icosahedron using its indices ****
//        renderEncoder.drawIndexedPrimitives(type: .triangle,
//                                            indexCount: indices.count, // Use Icosahedron index count (60)
//                                            indexType: .uint16,
...
//
//        // Let renderer configure pipeline based on view formats
...
//        // No specific updates needed from SwiftUI state in this example.
//    }
//}
//
...
//    var body: some View {
//        VStack(spacing: 0) {
//            Text("Rotating Wireframe Icosahedron (Metal)") // Updated Title
//                .font(.headline)
//                .padding()
//                .frame(maxWidth: .infinity)
//                .background(Color(red: 0.15, green: 0.1, blue: 0.1)) // Match clear color
//                .foregroundColor(.white)
//
//            MetalIcosahedronViewRepresentable() // Embed the Icosahedron Metal view
//                // .ignoresSafeArea() // Optionally uncomment
//        }
...
//
//#Preview {
//    // Option 1: Placeholder (Safer for Previews)
//    struct PreviewPlaceholder: View {
//        var body: some View {
//             VStack(spacing: 0) {
//                Text("Rotating Wireframe Icosahedron (Metal)") // Title match
//                    .font(.headline)
//                    .padding()
//                    .frame(maxWidth: .infinity)
//                    .background(Color(red: 0.15, green: 0.1, blue: 0.1))
...
//            .background(Color(red: 0.15, green: 0.1, blue: 0.1)) // Match BG
//            .edgesIgnoringSafeArea(.all)
//        }
//    }
//    return PreviewPlaceholder() // <-- Use placeholder by default
//
//    // Option 2: Attempt to Render Actual View (May Fail)
//    // return IcosahedronView() // <-- Uncomment this to try rendering
//}
//
//// MARK: - Matrix Math Helper Functions (using SIMD)
//// (These functions are generic and copied directly from the Octahedron example)
//
