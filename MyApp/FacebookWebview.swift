...
//        // MARK: - WKFindDelegate
//        @available(iOS 16.0, *)
//        func find(in webView: WKWebView, hasNextResult result: Bool, completionHandler: @escaping (Bool) -> Void) {
...
//          webView.find(searchText, configuration: WKFindConfiguration()) { [weak self] (result: WKFindResult) in
//              guard let self = self else { return }
...
//       func findPrevious() {
//           guard let webView = webView, !searchText.isEmpty else { return }
//           let options: WKFindConfiguration.Options = [.backwards, .caseInsensitive, .wrap]
//           webView.find(searchText, configuration: WKFindConfiguration(options: options)) { [weak self] (result: WKFindResult) in
//               guard let self = self else { return }
//               print("Find result: \(result)")
//               if !result.found {
