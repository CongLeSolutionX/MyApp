...
class Renderer: NSObject, MTKViewDelegate {
    ...
    func draw(in view: MTKView) {
        ...
        renderEncoder.setVertexBytes(&uniforms, length: MemoryLayout<Uniforms>.stride, index: 1) // Uniforms -> Buffer 1
        renderEncoder.setFragmentBytes(&uniforms, length: MemoryLayout<Uniforms>.stride, index: 0) // Uniforms -> Buffer 0
        ...
        if let buffer = skyVertexBuffer {
            ...
            renderEncoder.setFragmentBytes(&drawMode, length: MemoryLayout<Int32>.stride, index: 1) // Draw Mode -> Buffer 1
        ...
        // --- Draw Horizon Glow ---
         if let buffer = horizonVertexBuffer {
             ...
             // Render as thick line (might need geometry shader or manual thickening)
             // Or render as a line strip here, and the fragment shader simulates thickness/blur
             renderEncoder.setTriangleFillMode(.fill) // Ensure lines are visible? Or not needed for lines.
        ...
        if let buffer = starVertexBuffer, starInstanceCount > 0 {
             ...
             renderEncoder.setFragmentBytes(&drawMode, length: MemoryLayout<Int32>.stride, index: 1)
             // Point size is set in vertex shader via [[point_size]] based on data in buffer 0 (passed as SIMD4)
        ...
        // Don't wait synchronously in a real app unless necessary
