...
enum HomeReadyEvaluationAPIEndpoint {
    case evaluation
...
enum HomeReadyEvaluationAPIError: Error, LocalizedError {
    case invalidURL
...
struct HomeReadyEvaluationAPIAuthCredentials {
...
final class HomeReadyEvaluationService: ObservableObject {
    ...
    private func getAccessToken(completion: @escaping (Result<String, HomeReadyEvaluationAPIError>) -> Void) {
        ...
        let credentials = "\(HomeReadyEvaluationAPIAuthCredentials.clientID):\(HomeReadyEvaluationAPIAuthCredentials.clientSecret)"
        ...
        URLSession.shared.dataTaskPublisher(for: request)
            .tryMap { data, response -> Data in
                guard let httpResponse = response as? HTTPURLResponse,
                    ...
                    throw HomeReadyEvaluationAPIError.requestFailed("Invalid Response: \(responseString)")
                }
                return data
            }
            ...
            .sink { [weak self] completionResult in
                ...
                case .failure(let error):
                    let apiError = (error as? HomeReadyEvaluationAPIError) ?? HomeReadyEvaluationAPIError.unknown(error)
    ...
    private func makeEvaluationRequest(requestData: HomeReadyRequest, accessToken: String) {
        guard let url = URL(string: baseURLString + HomeReadyEvaluationAPIEndpoint.evaluation.path) else {
        ...
        URLSession.shared.dataTaskPublisher(for: request)
            ...
            .sink { [weak self] completionResult in
                ...
                case .finished:
                    break
                case .failure(let error):
                    self.handleError(error as? HomeReadyEvaluationAPIError ?? HomeReadyEvaluationAPIError.unknown(error))
                }
    ...
    // MARK: - Error Handling
    private func handleError(_ error: HomeReadyEvaluationAPIError) {
