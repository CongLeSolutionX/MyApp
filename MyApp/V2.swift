...
struct SignInWithAppleButtonView: UIViewRepresentable {
    ...
    // The presentation anchor window
    var window: UIWindow?
    ...
    class Coordinator: NSObject, ASAuthorizationControllerDelegate, ASAuthorizationControllerPresentationContextProviding {
        ...
        func presentationAnchor(for controller: ASAuthorizationController) -> ASPresentationAnchor {
            // Use the window passed down from the environment
            guard let window = parent.window else {
                // Fallback or handle error if window is not available
                // This should ideally not happen if environment is set up correctly
                fatalError("Presentation Anchor Window not found.")
            }
            return window
        }
    }
