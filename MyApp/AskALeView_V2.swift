...
//    // Note: Using AVAudioApplication.RecordPermission directly below, which conforms to CustomStringConvertible via extension
...
//        .onChange(of: isListening) { // Use new value syntax in Swift 5.5+ if needed: newValue in
//            updateInteractionMessage()
//        }
//        .onChange(of: micPermissionStatus){ // newValue in
//            updateInteractionMessage()
//        }
//        .onChange(of: speechPermissionStatus) { // newValue in
//            updateInteractionMessage()
//        }
...
//                        print("[State Change] isExperimentOn toggled to: \(isExperimentOn)") // Use newValue if needed
...
//                requestMicPermission() // Default to requesting if unknown
//            }
//        }
//    }
//
...
//        self.micPermissionStatus = currentMicPermission // Directly assign the enum value
//
...
//        // recognitionRequest.taskHint = .search // Optional context hint
...
//        // Use weak self in capture list to avoid potential retain cycles if self holds onto the tap block indirectly
...
//            self.recognitionRequest?.append(buffer) // Use optional chaining
//         }
//
...
//
//        // Check if the engine is actually running *before* trying to stop/remove tap
//        // Order Matters: Stop engine, then remove tap.
//        if audioEngine.isRunning {
//            print("  -> Stopping Audio Engine.")
//            audioEngine.stop()
//            print("  -> Audio Engine stopped.")
//
//            // Safely remove the tap from the input node
//            // Ensure the tap is removed only if it was installed
//            // (Checking audioEngine.isRunning implies it likely was, but be defensive)
...
//        // Only endAudio if the request object still exists
...
//        // cleanupListeningResources handles task cancellation, request release, session deactivation, and state update
//        cleanupListeningResources()
//    }
//
...
//        // Important to release audio hardware resources
//        // Use a separate do-catch for session deactivation
//        do {
//            // Check if session is active before trying to deactivate
//            // Checking category or other properties might be more reliable than secondaryAudioShouldBeSilencedHint
...
//        DispatchQueue.main.async {
//            // Only update state if it was previously 'listening' to avoid redundant updates
//            if self.isListening {
...
//extension SFSpeechRecognizerAuthorizationStatus: /*@retroactive - Removed as not needed if defined in same module */ CustomStringConvertible {
...
//}
//
//// Provides descriptive strings for AVAudioApplication.RecordPermission, useful for logging
//// REMOVED Redundant extension for AVAudioSession.RecordPermission
//
//// Provides descriptive strings for SFSpeechRecognitionTaskState, useful for debugging task lifecycle
//extension SFSpeechRecognitionTaskState: /*@retroactive - Removed */ @retroactive CustomStringConvertible {
//    public var description: String {
//        switch self {
...
//extension AVAudioApplication.recordPermission: /*@retroactive - Removed */ @retroactive CustomStringConvertible {
//    public var description: String {
//        switch self {
