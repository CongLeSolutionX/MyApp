...
//    
//    let creation = pthread_create(&thread, &attributes, { (argPointer: UnsafeMutableRawPointer?) -> UnsafeMutableRawPointer? in
//        // `argPointer` is now explicitly Optional `UnsafeMutableRawPointer?`
//        
//        guard let argPtr = argPointer else { // Use a different name to avoid shadowing if preferred, or reuse
//            print("Error: Received nil argPointer in thread creation callback")
//            return nil // Must return the expected type (UnsafeMutableRawPointer?)
//        }
//        // Retrieve, Cast, Call the block, and then release it
//        let unmanagedBlock = Unmanaged<AnyObject>.fromOpaque(argPtr)
//        let blockToRun = unmanagedBlock.takeUnretainedValue() as! () -> Void
//        blockToRun()
//        unmanagedBlock.release() // Release the block after execution
//        
//        return nil // pthread routine expects a return value (UnsafeMutableRawPointer?)
