...
// 1. Flatten the dictionary into an array of VoiceInfo objects
let flattenedVoices = mockLanguageVoices.flatMap { langCode, voiceNames in
    voiceNames.map { voiceName in
        VoiceInfo(name: voiceName, languageCode: langCode)
    }
}

// 2. Sort the flattened array using a more readable multi-line closure
let mockAvailableVoices: [VoiceInfo] = flattenedVoices.sorted { lhs, rhs in
    // Sort primarily by language code
    if lhs.languageCode != rhs.languageCode {
        return lhs.languageCode < rhs.languageCode
    }
    // If language codes are the same, sort by voice name
        ...
        NavigationView {
            ...
            .fileImporter(
                ...
                case .success(let urls):
                    // Attempt to get the first URL from the array
                    if let url = urls.first {
                        // Create a new Result with the single URL
                        viewModel.handleSelectedAudioPrompt(result: .success(url))
                    } else {
                        // Handle the (unlikely) case where the array is empty
                        viewModel.handleSelectedAudioPrompt(result: .failure(NSError(domain: "FileImporter", code: -1, userInfo: [NSLocalizedDescriptionKey: "No file URL received."])))
                    }
                case .failure(let error):
                    // Pass the error through directly
                    viewModel.handleSelectedAudioPrompt(result: .failure(error))
                }
                // --- FIX END ---
            }
            .fileImporter(
                ...
                case .success(let urls):
                    ...
                    if let url = urls.first {
                        // Create a new Result with the single URL
                        viewModel.handleSelectedDictionary(result: .success(url))
                    } else {
                        // Handle the (unlikely) case where the array is empty
                        viewModel.handleSelectedDictionary(result: .failure(NSError(domain: "FileImporter", code: -1, userInfo: [NSLocalizedDescriptionKey: "No file URL received."])))
                    }
                case .failure(let error):
                    // Pass the error through directly
                    viewModel.handleSelectedDictionary(result: .failure(error))
                }
                // --- FIX END ---
            }
