...
class FlowerOfLifeRenderer: NSObject, MTKViewDelegate {
    ...
    func updateState() {
        ...
        let timeSeedStart = 0.5 // Time the Seed of Life starts appearing
        let timeSeedDuration = 2.0 // Duration for Seed of Life circles to fade in
        let timeFlowerStart = timeSeedStart + timeSeedDuration + 0.5 // Time Flower starts
        let timeFlowerDuration = 3.0 // Duration for Flower of Life circles to fade in
        
        for i in 0..<maxInstances {
            if i >= circleCenters.count { continue } // Safety check
            
            var alpha: Float = 0.0
            let scale: Float = baseRadius // Use the base radius for scale
            ...
            if i == 0 { // Center circle always visible after start
                ...
                let startTimeForThis = timeSeedStart + Float(i-1) * (timeSeedDuration / 6.0) * 0.5 // Stagger start
                ...
                let startTimeForThis = timeFlowerStart + Float(i-7) * (timeFlowerDuration / 12.0) * 0.5 // Stagger start
                alpha = smoothstep(startTimeForThis, startTimeForThis + timeFlowerDuration * 0.8, currentTime)
            }
            
            if alpha > 0.001 { // Only update buffer and count if visible
            ...
            // Optimization: If we've determined the alpha for the last potential circle
            // and it's still 0, we don't need to check later ones for this frame.
            if alpha <= 0.001 && i >= 6 && currentTime < timeFlowerStart { // Break early if seed done and flower not started
                break
            }
            if alpha <= 0.001 && i > 0 && currentTime < timeSeedStart { // Break if center not yet visible
                break
            }
        }
        // The actual number of instances to draw is stored in instanceCount
        // We will pass this count to the draw call.
    }
    ...
    func draw(in view: MTKView) {
        ...
        updateState() // Update uniforms and instance data for animation
        
        // Determine how many instances to draw THIS FRAME based on animation progress
        // This needs to be calculated accurately in updateState() based on alpha values > 0
        var visibleInstanceCount = 0
        let instanceDataPtr = instanceDataBuffer.contents().bindMemory(to: InstanceData.self, capacity: maxInstances)
        let currentTime = Float(Date().timeIntervalSince(startTime)) // Recalc time locally or pass from updateState
        // --- Recalculate visibleInstanceCount logic (simplified example) ---
        // Define animation timings (redundant - Ideally calculate once in updateState)
        let timeSeedStart: Float = 0.5
        let timeSeedDuration: Float = 2.0
        // This calculation will now correctly result in a Float
        ...
        for i in 0..<maxInstances {
            var alpha: Float = 0.0
            if i == 0 {
                // 0.0 literal can work, smoothstep expects Float args
                alpha = smoothstep(0.0, timeSeedStart, currentTime)
            } else if i < 7 {
                // Cast integer result to Float for multiplication
                // Ensure literals 6.0 and 0.5 are treated as Float
                let startTimeForThis = timeSeedStart + Float(i-1) * (timeSeedDuration / Float(6.0)) * Float(0.5) // Fix Line 403 issues
                // Ensure literal 0.8 is Float
                alpha = smoothstep(startTimeForThis, startTimeForThis + timeSeedDuration * Float(0.8), currentTime) // Fix Line 404 issues
            } else {
                // Cast integer result to Float
                // Ensure literals 12.0 and 0.5 are treated as Float
                let startTimeForThis = timeFlowerStart + Float(i-7) * (timeFlowerDuration / Float(12.0)) * Float(0.5) // Fix Line 406 issues
                // Ensure literal 0.8 is Float
                alpha = smoothstep(startTimeForThis, startTimeForThis + timeFlowerDuration * Float(0.8), currentTime) // Fix Line 407 issues
            }
            
            // Ensure literal 0.001 is Float for compraison and the <= check below
            if alpha > Float(0.001) { visibleInstanceCount += 1 } // Fix for ">" comparison if needed
            else {
                // Ensure literal 0.001 is Float for comparison
                // Now comparing Float (currentTime) with Float (timeSeedStart/timeFlowerStart)
                if i>=0 && currentTime < timeSeedStart && alpha <= Float(0.001) {break} // Fix Line 412 issues
                if i>=6 && currentTime < timeFlowerStart && alpha <= Float(0.001) {break} // Fix Line 413 issues
            }
        }
        // --- End Recalculation ---
        ...
        if visibleInstanceCount > 0 { // Only draw if there's something visible
            ...
            renderEncoder.drawIndexedPrimitives(type: .lineStrip,                // Draw lines connecting vertices
                                                indexCount: circleIndices.count, // Vertices per circle instance
                                                indexType: .uint16,
                                                indexBuffer: circleIndexBuffer,
                                                indexBufferOffset: 0,
                                                instanceCount: visibleInstanceCount) // Draw this many instances
        }
