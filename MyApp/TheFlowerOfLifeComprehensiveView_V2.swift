...
class FlowerOfLifeRenderer: NSObject, MTKViewDelegate {
    ...
    func draw(in view: MTKView) {
        ...
        let timeSeedStart = 0.5
        let timeSeedDuration = 2.0
        let timeFlowerStart = timeSeedStart + timeSeedDuration + 0.5
        let timeFlowerDuration = 3.0

        for i in 0..<maxInstances {
             var alpha: Float = 0.0
             if i == 0 { alpha = smoothstep(0.0, timeSeedStart, currentTime) }
             else if i < 7 {
             ...
             if alpha > 0.001 { visibleInstanceCount += 1 }
             else {
                 // Optimization check - if alpha is zero don't bother checking further
                 if i>=0 && currentTime < timeSeedStart && alpha<=0.001 {break} // Before Seed
                 if i>=6 && currentTime < timeFlowerStart && alpha<=0.001 {break} // Before Flower
             }
        }
        // --- End Recalculation ---
