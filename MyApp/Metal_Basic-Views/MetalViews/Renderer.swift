...
final class CubeRenderer: Renderer {
  ...
  init?(device: any MTLDevice) {
    ...
    guard let library = device.makeDefaultLibrary(),
          let vertexFunc = library.makeFunction(name: "main_vertex_for_3D_view"),
          let fragmentFunc = library.makeFunction(name: "main_fragment_for_3D_view") else { return nil }
    ...
    // Setup buffers. Coordinates defined in clip space coords: [-1,+1] for x and y; and [0,+1] for z.
      let vertices: [ShaderVertexFor3DView] = [
        ShaderVertexFor3DView(position: [-1,  1,  1, 1], color: [0, 1, 1, 1]), // left,  top,    back
        ShaderVertexFor3DView(position: [-1, -1,  1, 1], color: [0, 0, 1, 1]), // left,  bottom, back
        ShaderVertexFor3DView(position: [ 1, -1,  1, 1], color: [1, 0, 1, 1]), // right, bottom, back
        ShaderVertexFor3DView(position: [ 1,  1,  1, 1], color: [1, 1, 1, 1]), // right, top,    back
        ShaderVertexFor3DView(position: [-1,  1, -1, 1], color: [0, 1, 0, 1]), // left,  top,    front
        ShaderVertexFor3DView(position: [-1, -1, -1, 1], color: [0, 0, 0, 1]), // left,  bottom, front
        ShaderVertexFor3DView(position: [ 1, -1, -1, 1], color: [1, 0, 0, 1]), // right, bottom, front
        ShaderVertexFor3DView(position: [ 1,  1, -1, 1], color: [1, 1, 0, 1])  // right, top,    front
    ]

      guard let verticesBuffer = device.makeBuffer(bytes: vertices, length: vertices.count * MemoryLayout<ShaderVertexFor3DView>.stride, options: .cpuCacheModeWriteCombined) else { return nil }
    ...
    self.indecesBuffer = indicesBuffer.configure { $0.label = .identifier(Self.id, "buffer.indices") }
      ...
      guard let uniformsBuffer = device.makeBuffer(length: MemoryLayout<ShaderUniformsFor3DView>.stride) else { return nil }
...
extension CubeRenderer {
  func draw(layer: CAMetalLayer, time: (now: Double, display: Double)) {
    ...
    case .none: self.uniforms = Uniforms(now: time.now, display: time.display)
      ...
      let ptr = self.uniformsBuffer.contents().assumingMemoryBound(to: ShaderUniformsFor3DView.self)
      ptr.pointee = ShaderUniformsFor3DView(mvpMatrix: self.uniforms!.projectionMatrix(size: size))
