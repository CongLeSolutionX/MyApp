//
//  RendererForTexturingView.swift
//  MyApp
//
...
@MainActor final class CowRenderer: NSObject, MTKViewDelegate {
    ...
    private let diffuseTexture: any MTLTexture
    private let textureSampler: any MTLSamplerState
    ...
    init?(device: any MTLDevice) {
        ...
        guard let library = device.makeDefaultLibrary(),
              let vertexFunc = library.makeFunction(name: "main_vertex_for_texturing_view"),
              let fragmentFunc = library.makeFunction(name: "main_fragment_for_texturing_view") else { return nil }
        ...
        guard let uniformBuffer = device.makeBuffer(length: MemoryLayout<ShaderUniformsForTexturingView>.stride) else { return nil }
    ...
    nonisolated func draw(in view: MTKView) {
        MainActor.assumeIsolated {
            ...
            self.uniformsBuffer.contents()
                .assumingMemoryBound(to: ShaderUniformsForTexturingView.self)
                .pointee = ShaderUniformsForTexturingView(modelViewProjectionMatrix: matrix.projection, modelViewMatrix: matrix.modelView, normalMatrix: matrix.normal)
