...
class GoogleBooksAPIService: ObservableObject {
    ...
    func fetchBooks(query: String, filter: String? = nil, printType: String = "all", orderBy: String = "relevance", maxResults: Int = 10, startIndex: Int = 0) async throws -> [BookItem] {
        ...
        // 1. Calculate the encoded query string from the assigned queryItems first.
        let encodedQueryString = components?.queryItems?.map { item in
        ...
        // 2. Assign the pre-calculated string back to the components.
        components?.percentEncodedQuery = encodedQueryString
