...
class BrowserViewModel {
    ...
    func loadURL(string: String) {
        browserError = nil // Clear previous errors
        guard let url = validatedURL(from: string) else {
            browserError = .invalidURLError(urlString: string) // Set error
            return
        }
    ...
    func loadStartPage() {
        ...
        guard let startURL = Bundle.main.url(forResource: "UserAgent", withExtension: "html") else {
            browserError = .loadingError(underlyingError: NSError(domain: "BrowserViewModel", code: -1, userInfo: [NSLocalizedDescriptionKey: "Start page not found"])) // Set error
            return
        }
    ...
    private func handleWebViewError(_ error: Error) {
        ...
        if nsError.code != NSURLErrorCancelled {
            ...
            if nsError.domain == NSURLErrorDomain {
                browserError = .networkError(underlyingError: error)
            } else {
                browserError = .loadingError(underlyingError: error)
            }
        }
    }
}
...
class ShinnyBrowserViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        setupUI()
        setupConstraints()
        setupErrorHandling() // Setup error handling observation
    ...
    // MARK: - Error Alert Presentation
      ...
      func presentErrorAlert(error: BrowserError) {
