...
class BrowserViewModel {
    ...
    init(webView: WKWebView = WKWebView(frame: .zero, configuration: WKWebViewConfiguration())) {
        ...
        loadContentModePreferences() // Load preferences during initialization
        setupBindings()
    }
    ...
    private func loadContentModePreferences() {
        if let savedPreferences = UserDefaults.standard.dictionary(forKey: contentModePreferencesKey) as? [String: Int] {
            contentModeToRequestForHost = savedPreferences.compactMapValues { rawValue in
                WKWebpagePreferences.ContentMode(rawValue: rawValue)
            }
        }
    }

    private func saveContentModePreferences() {
        let preferencesToSave = contentModeToRequestForHost.mapValues { $0.rawValue }
        UserDefaults.standard.set(preferencesToSave, forKey: contentModePreferencesKey)
    }
...
//            .assign(to: \.text, on: urlField) // keep this line commented out
