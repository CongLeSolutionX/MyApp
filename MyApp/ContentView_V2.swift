...
//struct GeminiModel: Identifiable, Equatable {
//    let id = UUID()
//    let name, identifier, inputs, outputs, optimizedFor: String
//    let imageName: String?
//    var placeholderColor: Color {
...
let geminiModelsData: [GeminiModel] = [
    GeminiModel(
        ...
        imageName: "https://www.pexels.com/photo/green-palm-tree-near-white-and-black-dome-building-under-blue-sky-9002742/",
        identifier: "gemini-2.5-pro-exp-03-25",
        inputs: "Audio, images, videos, and text",
        outputs: "Text",
        optimizedFor: "Enhanced thinking and reasoning, multimodal understanding, advanced coding, and more"
    ...
    // ... other Gemini models remain unchanged ...
]
...
struct GeminiCardView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                // Display image if available (remote or local)
                if let imageName = model.imageName {
                    imageView(for: imageName)
                        .clipShape(RoundedRectangle(cornerRadius: isMinimized ? 20 : 30))
                        .frame(maxWidth: .infinity, maxHeight: 150)
    ...
    @ViewBuilder
    private func imageView(for imageName: String) -> some View {
        // Check if imageName is a valid remote URL using HTTP/HTTPS scheme
        if let url = URL(string: imageName),
           let scheme = url.scheme,
           scheme.lowercased() == "http" || scheme.lowercased() == "https" {
            AsyncImage(url: url) { phase in
...
struct ContentView: View {
    ...
    var body: some View {
        VStack {
            VStack(alignment: .leading, spacing: 8) {
                // Usage instructions or additional content…
            }
            .padding([.horizontal, .top], 15)
            .padding(.bottom, 15)
