...
/// Fragment shader for visualizing a scalar field with a wider color palette.
/// This shader maps scalar values to a color scale ranging from blue to green to yellow.
fragment half4 visualizeScalar(VertexOut fragmentIn [[stage_in]], texture2d<float, access::sample> tex2d [[texture(0)]]) {
    constexpr sampler sampler2d(filter::nearest);

    float value = tex2d.sample(sampler2d, fragmentIn.textureCoorinates).x;
    float absoluteValue = abs(value);

    // Map the absolute value to the range [0, 1] (or adjust as needed)
    float normalizedValue = clamp(absoluteValue, 0.0, 1.0); // Example clamping

    // Define colors for the gradient
    half3 blue = half3(0.0, 0.06, 0.19);
    ...
    if (normalizedValue < 0.5) {
        color = mix(blue, green, normalizedValue * 2.0);
    } else {
        color = mix(green, yellow, (normalizedValue - 0.5) * 2.0);
    }
...
/// Fragment shader for visualizing a vector field with a diverging color scale.
/// This shader maps vector magnitudes to a color scale diverging from green (negative) to gray (zero) to red (positive).
fragment half4 visualizeVector(VertexOut fragmentIn [[stage_in]], texture2d<float, access::sample> tex2d [[texture(0)]]) {
    constexpr sampler sampler2d(filter::nearest);

    float3 value = tex2d.sample(sampler2d, fragmentIn.textureCoorinates).xyz;
    ...
    // Interpolate between colors based on the sign and magnitude of the vector
    half3 color;
    if (value.x > 0.0) { // Positive values (red)
        color = mix(gray, red, clamp(magnitude, 0.0, 1.0));
    } else { // Negative values (green) or zero (gray)
        color = mix(gray, green, clamp(magnitude, 0.0, 1.0));
    }
