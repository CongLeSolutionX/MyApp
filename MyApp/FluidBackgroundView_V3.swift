...
//    
#if DEBUG // Or a more specific check for previews if available
let jacobiIterations = 5
let diffusionIterations = 2
#else
let jacobiIterations = 20
let diffusionIterations = 8
#endif
    ...
    func draw(in view: MTKView) {
        ...
        if fluidViscosity > 0 && diffusionIterations > 0 {
            ...
            var vel_b = velocityTextureA! // Input 'b' term (velocity before diffusion)
        ...
        // 5b. Solve Pressure (Iteratively A <-> B using Divergence) -> Final pressure in A or B
         ...
         blitTexture(commandBuffer: commandBuffer, source: pressureTextureA, destination: pressureTextureA, clear: true)
         blitTexture(commandBuffer: commandBuffer, source: pressureTextureB, destination: pressureTextureB, clear: true)
        ...
        guard let renderPassDescriptor = view.currentRenderPassDescriptor else {
             ...
             // Command buffer will be committed without rendering. Compute work is still done.
             commandBuffer.commit()
             return
        }
        // Load action is implicitly handled by view.clearColor set during init
        ...
        // Reset one-shot interaction flags if needed (better done in interaction methods)
    ...
    func blitTexture(commandBuffer: MTLCommandBuffer, source: MTLTexture, destination: MTLTexture, clear: Bool = false) {
        guard let blitEncoder = commandBuffer.makeBlitCommandEncoder() else {
             print("Error: Failed to create blit command encoder for texture copy/clear.")
             return
         }
         blitEncoder.label = "Blit Texture (\(source.label ?? "src") -> \(destination.label ?? "dst"))"

         if clear {
             // Simple clear using compute shader is preferred now via clearTexture()
             // This blit clear might not be efficient or fully correct depending on format.
             // Keeping it as a fallback concept, but recommend using clearTexture() with compute.
             print("Warning: Using blitEncoder clear - compute shader clear is preferred.")
             blitEncoder.fill(buffer: device.makeBuffer(length: 1)!, range: 0..<1, value: 0) // Fill with zero
             blitEncoder.copy(from: device.makeBuffer(length: 1)!, sourceOffset: 0, sourceBytesPerRow: 0, sourceBytesPerImage: 0, sourceSize: MTLSize(width: 0, height: 0, depth: 0), to: destination, destinationSlice: 0, destinationLevel: 0, destinationOrigin: MTLOrigin(x: 0, y: 0, z: 0))
         } else {
...
struct FluidBackgroundView: UIViewRepresentable {
    ...
    func makeCoordinator() -> Coordinator {
        // Attempt to create the renderer first
        guard let device = MTLCreateSystemDefaultDevice(),
             ...
             // maybe by showing an error message in the view.
    ...
    func makeUIView(context: Context) -> MTKView {
        ...
        // Important: Ensure the view has delegate and device set *before* returning
        return mtkView
    }
    ...
    class Coordinator: NSObject, MTKViewDelegate {
        ...
        @objc func handleTap(_ gesture: UITapGestureRecognizer) {
            ...
            // Treat tap as a single point interaction (not dragging)
            renderer.updateInteraction(point: location, viewSize: viewSize, isDragging: false)
             // Maybe call endInteraction directly after a tap if it behaves like an impulse?
             // Or rely on the draw loop picking up addDensity=true for one frame.
             // Let's keep flags active for one frame after tap.
        }
    }
}
...
struct ContentView: View {
    var body: some View {
        ZStack {
            // FluidBackgroundView as the background layer
            FluidBackgroundView()
            ...
            VStack {
                Text("Interactive Fluid")
                    ...
                    .padding(.top, 60) // Adjust for notch/status bar
         ...
         // Consider setting preferred color scheme if needed
