...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication,
        ...
        //    File > Add Packages... > https://github.com/googleads/swift-package-manager-google-mobile-ads.git
        // 2. Update your Info.plist:
        //    - Add GADApplicationIdentifier (String) with YOUR AdMob App ID from the AdMob UI.
        //      (Based on your image, use: "ca-app-pub-7804958538882040~1154799997")
        //    - Add SKAdNetworkItems (Array) with the dictionary entries provided
        //      in the Google AdMob documentation.
        // -----------------------------------------

        // Configure Test Devices (Programmatic Method - Recommended during development)
        // -> See "Enabling Test Ads" Card in the UI for detailed instructions.
        // !! IMPORTANT !!: Since you'll be using a REAL Ad Unit ID below,
        // enable your test device ID here to see the "Test Ad" label and safely click ads.
        // Otherwise, DO NOT click the ads shown by your real Ad Unit IDs during testing.
        // GADMobileAds.sharedInstance().requestConfiguration.testDeviceIdentifiers = [ "YOUR_SIMULATOR_OR_DEVICE_ID_HERE" ]
            ...
            ScrollView {
                VStack(spacing: 20) {
                    ...
                    AdFormatCardView(
                        ...
                        **Methods:**
                        1.  **Demo Ad Units:** Use Google's provided *test* ad unit IDs (like `ca-app-pub-3940.../...).` Not linked to your account.
                        2.  **Test Devices (Using Your Real Ad Units):** Use your *real* ad unit IDs (like the banner below, `ca-app-pub-7804.../...`) but designate specific devices/simulators for testing in the `AppDelegate`:
                        ```swift
                        // In AppDelegate, before SDK start():
                        GADMobileAds.sharedInstance()
                           .requestConfiguration
                           .testDeviceIdentifiers = [ "YOUR_ID_HERE" ]
                        ```
                        - Run app, find ID in console, add it, rerun.
                        - Google ads will show a **\"Test mode\"** label and are safe to click.
                        
                        **Mediation Note:** Test label only on Google ads. Enable test mode for *each* mediation network separately.
                        """,
                        iconName: "wrench.and.screwdriver.fill"
                    ) /* No ad content needed for info card */

                    // --- Ad Format Cards ---

                    AdFormatCardView(
                        title: "Banner Ads",
                        description: "Rectangular ads fitting a portion of the layout. Can auto-refresh.",
                        iconName: "rectangle.on.rectangle",
                        adContent: {
                            // Using the specific Ad Unit ID from your screenshot
                            // REMEMBER TO ENABLE TEST DEVICES TO SAFELY CLICK THIS
                            BannerAdView()
                                .frame(height: 50)
                        }
                    )

                    // --- Other Ad Format Placeholders (Unchanged, still need specific Ad Unit IDs) ---
                    ...
                    AdFormatCardView(
                        ...
                        adContent: {
                            Button("Show Interstitial (Placeholder)") {
                                print("Interstitial: Needs its own Ad Unit ID (Demo or Real + Test Device).")
                                // Needs GADInterstitialAd.load with ITS OWN ad unit ID, then present.
                            }
                            .buttonStyle(.borderedProminent)
                            .tint(.blue)
                        }
                    ...
                    AdFormatCardView(
                        ...
                        iconName: "list.bullet.rectangle.portrait",
                        adContent: {
                           Text("Native Ad Placeholder\n(Needs its own Ad Unit ID)")
                                .font(.caption)
                                .multilineTextAlignment(.center)
                                .foregroundColor(.secondary)
                                .frame(minHeight: 50)
                               // Needs GADAdLoader with ITS OWN ad unit ID, delegate, and custom UI binding.
                               // Headline prepended with "Test mode" if using test device + real ID.
                        }
                    )

                    AdFormatCardView(
                        title: "Rewarded Ads",
                        description: "Ads users watch voluntarily for in-app rewards. Requires opt-in.",
                        iconName: "gift",
                        adContent: {
                            Button("Show Rewarded Ad (Placeholder)") {
                                print("Rewarded: Needs its own Ad Unit ID (Demo or Real + Test Device).")
                                // Needs GADRewardedAd.load with ITS OWN ad unit ID, present, handle reward.
                            }
                            .buttonStyle(.borderedProminent)
                            .tint(.green)
                        }
                    ...
                    AdFormatCardView(
                        ...
                        adContent: {
                            Button("Show Rewarded Interstitial (Placeholder)") {
                                print("Rewarded Interstitial: Needs its own Ad Unit ID (Demo or Real + Test Device).")
                                // Needs GADRewardedInterstitialAd.load with ITS OWN ad unit ID, present, handle reward.
                            }
                            .buttonStyle(.borderedProminent)
                            .tint(.orange)
                        }
                    ...
                    AdFormatCardView(
                        title: "App Open Ads",
                        description: "Ads shown when the app is opened or switched back to.",
                        iconName: "rectangle.portrait.on.rectangle.portrait",
                        adContent: {
                           Text("App Open Ad Placeholder\n(Needs its own Ad Unit ID)")
                                .font(.caption)
                                ...
                                .foregroundColor(.secondary)
                                .frame(minHeight: 50)
                                // Needs GADAppOpenAd.load with ITS OWN ad unit ID and show logic.
                        }
                    )
                }
                .padding()
...
// MARK: - Reusable Card View (Unchanged from previous response)
...
// MARK: - Banner Ad Implementation (UIViewControllerRepresentable) - UPDATED

struct BannerAdView: UIViewControllerRepresentable {

    // --- Using YOUR Specific Ad Unit ID from the Screenshot ---
    // IMPORTANT: This is a REAL Ad Unit ID from your account.
    // You MUST enable test devices in AppDelegate OR avoid clicking this ad during testing
    // to prevent your account from being flagged for invalid activity.
    private let adUnitID = "ca-app-pub-7804958538882040/6269828630"
...
// MARK: - BannerViewController (Helper - Unchanged logic, uses ID passed to it)
...
class BannerViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        guard let adUnitID = adUnitID else {
            print("Error: BannerViewController requires an adUnitID.")
            return
        }
...
// MARK: - Banner View Delegate (Unchanged logic)

extension BannerViewController: BannerViewDelegate {
    func bannerViewDidReceiveAd(_ bannerView: BannerView) {
      print("Banner Ad: Received ad successfully.")
        if let adInspectorClassName = bannerView.adUnitID, adInspectorClassName == "GADMAdapterGoogleAdMobAds" {
             print("Banner Ad: Served by Google AdMob network. Check visually for 'Test mode' label if using test devices with your real Ad Unit ID.")
         } else {
             print("Banner Ad: Served by network: \(bannerView.adUnitID ?? "Unknown"). No 'Test mode' label will appear. Ensure test mode enabled for this network if applicable.")
         }
    }
...
struct GoogleAdMobIntegrationView2_Previews: PreviewProvider {
    static var previews: some View {
        GoogleAdMobIntegrationView2()
            // Note: Previews might still crash if Info.plist isn't set up,
            // or might show placeholder ads if network fails in preview environment.
            // Test Device ID mechanism might also behave differently in Previews vs. Simulator/Device run.
    }
}
