             ...
             for await notification in notifications {
                ...
                await MainActor.run {
                    ...
                    // --- Safer approach: Assume any notification means potential change ---
                     // If no specific keys are found, assume both might need reload
                    if !(needsEventReload || needsReminderReload) {
                         print("No specific change key found in notification, reloading both for safety.")
                         needsEventReload = true
                         needsReminderReload = true
                    }
                    // --- End Safer Approach ---
        ...
        NavigationStack {
            List {
                ...
                Section("Calendar Events") {
                    ...
                    } else if !eventManager.eventsAuthorized {
                         Text("Events access not granted.")
                             .foregroundColor(.secondary)
                    } else {
                ...
                Section("Reminders") {
                    ...
                    if !eventManager.incompleteReminders.isEmpty {
                        ...
                        ForEach(eventManager.incompleteReminders) { reminder in
                         ...
                         Text("Reminders access not granted.")
                            .foregroundColor(.secondary)
                    } else if eventManager.reminders.isEmpty && eventManager.remindersAuthorized {
                        Text("No reminders found.")
                           .foregroundColor(.secondary)
                    } else if eventManager.incompleteReminders.isEmpty && eventManager.remindersAuthorized {
                    ...
                    // Optionally display completed reminders
                     ...
                     .disabled(!eventManager.remindersAuthorized)
                }
            }
...
//             .onChange(of: eventManager.currentError) {
...
// Make EKEvent identifiable using its own unique identifier
extension EKEvent: Identifiable {
    // EKEvent already has eventIdentifier which is unique and suitable for Identifiable
    // However, ForEach might still complain if the conformance isn't explicitly available at compile time sometimes
    // Explicitly defining `id` using `eventIdentifier` can help resolve this.
    public var id: String { self.eventIdentifier }
}

// Make EKReminder identifiable using its calendarItemIdentifier
extension EKReminder: Identifiable {
     // EKReminder uses calendarItemIdentifier inherited from EKCalendarItem
...
struct ReminderRow: View {
    ...
    var body: some View {
        HStack {
            ...
            if reminder.priority > 0 {
                Text("\(prioritySymbol(reminder.priority))")
                    .font(.caption)
                    .padding(.horizontal, 3)
                    .foregroundColor(priorityColor(reminder.priority))
            }

             // Display Calendar Source Dot
             Circle()
                  .fill(Color(cgColor: reminder.calendar.cgColor))
    ...
    func priorityColor(_ priority: Int) -> Color {
        switch priority {
        case 1...4: return .red // High
        case 5: return .orange // Medium
        case 6...9: return .blue // Low
        default: return .gray
        }
    }
    // Helper for priority symbol
    func prioritySymbol(_ priority: Int) -> String {
        switch priority {
...
// Make EKCalendar usable in Picker/ForEach by identifying it
// While we use `id: \.calendarIdentifier` in ForEach, making the class itself
// conform can sometimes simplify things, though not strictly required with the explicit id.
// Pickers often work better if the selection *type* is Hashable.
extension EKCalendar: Identifiable, Hashable {
     public var id: String { self.calendarIdentifier }
...
//     }
     ...
     public static func == (lhs: EKCalendar, rhs: EKCalendar) -> Bool {
...
struct AddEventView: View {
    ...
    @State private var isAllDay = false
     // Use EKCalendar for selection type now that it's Hashable
        ...
        NavigationView {
            Form {
                 ...
                 Picker("Calendar", selection: $selectedCalendar) {
                     Text("None Selected").tag(nil as EKCalendar?) // Option for no selection initially
                     // Use explicit identifier for ForEach still
                     ForEach(eventManager.eventCalendars, id: \.calendarIdentifier) { calendar in
                         ...
                         .tag(calendar as EKCalendar?) // Tag type matches selection type
                     }
                 }
                ...
                if !isAllDay {
                    DatePicker("End Date", selection: $endDate, displayedComponents: [.date, .hourAndMinute])
                        ...
                        .onChange(of: startDate) {oldDate, newDate in
                             if endDate < newDate {
                                 endDate = newDate.addingTimeInterval(3600) // Reset end date if start moves past it
                             }
                         }
                         .onChange(of: endDate) {oldDate, newDate in
                             if newDate < startDate {
                                 endDate = startDate // Don't allow end before start
                             }
                         }
                }
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                     Button("Save") { saveEvent() }.disabled(title.isEmpty || selectedCalendar == nil)
                 }
            }
    ...
    func saveEvent() {
        guard let calendar = selectedCalendar else {
             print("Cannot save event: No calendar selected.")
        ...
        Task {
           await eventManager.createNewEvent(
            ...
            if eventManager.currentError == nil { // Dismiss only if save succeeded (or no error was set)
             ...
             // If error occurred, the main view's alert should handle showing it
        }
    }
}
...
struct AddReminderView: View {
    ...
    var body: some View {
       NavigationView {
            Form {
                 ...
                 Picker("List", selection: $selectedCalendar) {
                    Text("None Selected").tag(nil as EKCalendar?)
                     // Use explicit identifier for ForEach
                ...
                TextField("Notes (Optional)", text: $notes, axis: .vertical)
                 ...
                 Picker("Priority", selection: $priority) {
                     Text("None").tag(0)
                     Text("Low (!)").tag(9)       // Example mapping from Reminders app
                     Text("Medium (!!)").tag(5)
                     Text("High (!!!)").tag(1)
                 }
                 .pickerStyle(.segmented) // Looks better for priority
             ...
             .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") { saveReminder() }.disabled(title.isEmpty || selectedCalendar == nil)
                 }
            }
       }
    }
     ...
     func saveReminder() {
         guard let calendar = selectedCalendar else {
            print("Cannot save reminder: No list selected.")
            return
        }
