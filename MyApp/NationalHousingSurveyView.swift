...
final class NationalHousingSurveyService: ObservableObject {
    ...
    // No longer need 'tokenHeader' constant as we use the specific header name directly below
    ...
    // ... (getAccessToken function remains the same as in the previous response, including debug logs and credential warnings) ...
        ...
        URLSession.shared.dataTaskPublisher(for: request)
            ...
            .sink { [weak self] completionResult in
                ...
                case .failure(let error):
                    ...
                    // Ensure UI update and completion call on main thread
                ...
                // Now call completion AFTER caching
        ...
        getAccessToken { [weak self] result in
            ...
            DispatchQueue.main.async {
                ...
                case .failure(let error): // Error already handled in getAccessToken
                    ...
                    self.isLoading = false // Ensure loading stops if token fetch fails
                }
            }
        }
    }
    ...
    private func makeDataRequest(endpoint: NationalHousingSurveyViewAPIEndpoint, accessToken: String) {
        ...
        request.addValue("application/json", forHTTPHeaderField: "Accept") // Ask for JSON response
        
        // --- CORRECTED HEADER USAGE ---
        // The API requires the token in the 'x-public-access-token' header for data requests.
        request.addValue(accessToken, forHTTPHeaderField: "x-public-access-token")
        // -----------------------------
        
        let tokenSnippet = accessToken.prefix(8)
        // Updated log to show the correct header being used
        print("[Network Debug] Making data request: URL=\(url), Method=\(request.httpMethod!), Headers=\(request.allHTTPHeaderFields?.description ?? "None"), Token=x-public-access-token: \(tokenSnippet)...")
        
        URLSession.shared.dataTaskPublisher(for: request)
            .tryMap { [weak self] data, response -> Data in // Use weak self
                guard let httpResponse = response as? HTTPURLResponse else {
                ...
                case 401:
                    ...
                    // Invalidate the token used for this failed request
                    DispatchQueue.main.async {
                        self?.accessToken = nil // Use optional chaining
            ...
            .sink { [weak self] completionResult in
                ...
                // Always ensure loading is stopped on the main thread after completion/failure
                DispatchQueue.main.async {
                ...
                case .failure(let error):
                    ...
                    self.handleError(apiError) // Already ensures main thread
                }
                ...
                do {
                    ...
                    if responseType == [NhsResults].self {
                        ...
                        throw NationalHousingSurveyViewAPIError.decodingFailed // Will be caught below
                    }
                    self.errorMessage = nil // Clear error on success
                    // No need to set isLoading false here, handled in completion block
                    ...
                    self.handleError(NationalHousingSurveyViewAPIError.decodingFailed) // Already ensures main thread
                    // No need to set isLoading false here, handled in completion block
                }
            }
            .store(in: &cancellables)
    ...
    // MARK: - Helpers (determineResponseType, clearLocalData, handleError)
